...
CompassHelper.prototype = {
	...
	_handleWatch: function() {
		...
		if (that.watchID !== null) {
			...
			that.watchID = null;
			button.innerHTML = "Старт на Компаса";
			...
			that._clearCurrentNotification();
			that._writeNotification("Очакване на информация от компаса...");
			button.innerHTML = "Стоп на Компаса";
	...
	_displayHeading: function(heading) {
		...
		var informationMessage = 'Магнитно поле: ' + magneticHeading + '<br />' +
								 'Време: ' + timestamp + '<br />' 
	...
	_onCompassWatchError: function(error) {
		...
		switch (error.code) {
			case 20:
				errorMessage = "Компаса не се поддържа";
				break;
			case 0:
				errorMessage = "Компас вътрешна грешка";
				break;
			default:
				errorMessage = "Компас грешка";
		}
